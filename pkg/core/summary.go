...
type InstallSummary struct {
	...
	reinstalls []PackageInfo // Packages being reinstalled
...
func NewInstallSummary() *InstallSummary {
	return &InstallSummary{
		installs:   make([]PackageInfo, 0),
		reinstalls: make([]PackageInfo, 0),
...
func (s *InstallSummary) AddInstall(pkg PackageInfo) {
	s.installs = append(s.installs, pkg)
}

// AddReinstall adds a package to be reinstalled
func (s *InstallSummary) AddReinstall(pkg PackageInfo) {
	s.reinstalls = append(s.reinstalls, pkg)
}

// AddUpgrade adds a package to be upgraded
func (s *InstallSummary) AddUpgrade(pkg PackageInfo, oldVersion string) {
...
func (s *InstallSummary) GetInstalls() []PackageInfo {
	return s.installs
}
...
func (s *InstallSummary) GroupBySource() map[string][]PackageInfo {

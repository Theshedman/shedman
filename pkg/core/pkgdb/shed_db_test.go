...
func TestShedDB_Search(t *testing.T) {
	...
	db.SetHTTPClient(func(url string) ([]byte, error) {
		response := pkgdb.ShedSearchResponse{
			Packages: []pkgdb.ShedPackage{
				{
	...
	if results[0].Source != pkgdb.SourceShedOS {
...
func TestShedDB_GetInfo(t *testing.T) {
	...
	db.SetHTTPClient(func(url string) ([]byte, error) {
		response := pkgdb.ShedInfoResponse{
			Package: &pkgdb.ShedPackage{
				Name:          "shedos-core",
				Version:       "2.0.0-1",
...
func TestShedDB_HTTPError(t *testing.T) {
	...
	db.SetHTTPClient(func(url string) ([]byte, error) {
		return nil, pkgdb.ErrShedRequestFailed
	})
...
func TestShedDB_ShedPackageToPackageInfo(t *testing.T) {
	...
	info := pkgdb.ShedPackageToPackageInfo(shedPkg)
